...
**No CAPTCHA yet, deliberately.** The honeypot field (`website`, silently
discarded if filled) is the only bot defence today. Turnstile is being
wired separately via Cloudflare's Turnstile Spin flow — the form and the
`/api/contact` handler are written with that in mind (a token field slots
into the form, a verification call slots into the handler before the
validation checks run) but neither exists yet. Don't add Turnstile here;
that's the next step, not this one.
